...
export default function ContactPage() {
  return (
    <section className="relative overflow-hidden pb-24 pt-36 lg:pb-32 lg:pt-44">
      ...
      <div className="absolute -left-40 top-10 -z-10 size-[500px] rounded-full bg-emerald-200/50 blur-3xl" />
        ...
        <div className="grid items-start gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
          <Reveal>
            ...
            <div className="mt-9 space-y-4">
              ...
              ].map((item) => <div key={item.text} className="flex items-center gap-3 text-sm font-semibold text-slate-700"><span className="flex size-9 items-center justify-center rounded-xl bg-emerald-100 text-emerald-700"><item.icon className="size-4" /></span>{item.text}</div>)}
            </div>
            <div className="mt-10 rounded-2xl border border-slate-200 bg-white/70 p-5">
              ...
              <a href="mailto:contato@zimobi.com.br" className="mt-4 flex items-center gap-3 text-sm font-semibold text-slate-800 hover:text-emerald-700"><Mail className="size-4 text-emerald-600" /> contato@zimobi.com.br</a>
              <p className="mt-4 flex items-start gap-3 text-sm leading-6 text-slate-600"><MapPin className="mt-1 size-4 shrink-0 text-emerald-600" /> Av. Cândido de Abreu, 381 — Centro Cívico, Curitiba — PR</p>
            </div>
          </Reveal>
